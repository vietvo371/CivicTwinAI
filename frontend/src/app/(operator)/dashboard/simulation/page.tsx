          ...
          <Card className="bg-card/50 backdrop-blur-xl shadow-lg border-border/80">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isRunning ? (
                    <span className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> {t('op.running')}</span>
                  ) : (
                    <span className="flex items-center gap-2"><Play className="w-4 h-4" /> {t('op.runSimulation')}</span>
                  )}
                </Button>
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-background/50 p-3 rounded-lg border border-border/50 text-center">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{t('op.edgesAffected')}</p>
                    <p className="text-2xl font-heading font-black text-violet-500">{result.affected_edges}</p>
                  </div>
                  <div className="bg-background/50 p-3 rounded-lg border border-border/50 text-center">
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{t('op.runtimeLabel')}</p>
                    <p className="text-2xl font-heading font-black text-blue-500">{(result.duration_ms / 1000).toFixed(1)}s</p>
                  </div>
                </div>
                <div className="bg-background/50 p-3 rounded-lg border border-border/50">
                  <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-2">{t('op.avgDensityChange')}</p>
                  <div className="flex items-end gap-3">
                    <div>
                      <p className="text-xs text-muted-foreground">{t('op.before')}</p>
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground">{t('op.after')}</p>
                ...
                {isRunning && (
                  <div className="absolute inset-0 bg-background/70 backdrop-blur-sm flex flex-col items-center justify-center gap-4 z-20">
                    ...
                    <div className="text-center">
                      <p className="font-heading font-bold text-lg">{t('op.simulatingImpact')}</p>
                      <p className="text-sm text-muted-foreground mt-1">{t('op.gnnProcessing')}</p>
                    </div>
                  </div>
                )}
              </div>
                  ...
                  {result.segments.map((seg, i) => (
                    <div key={i} className="bg-background/50 border border-border/50 rounded-xl p-4 hover:border-border transition-colors">
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-3">
                        <div>
                          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{t('op.before')}</p>
                          <p className={`text-lg font-heading font-bold ${getDensityColor(seg.before)}`}>
                            {(seg.before * 100).toFixed(0)}%
                          </p>
                        </div>
                        <div>
                          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">{t('op.after')}</p>
