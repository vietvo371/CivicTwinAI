...
import { aiService, mapService, mediaService, reportService } from '../../services';
...
const EditReportScreen = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setFormData({
          mo_ta: report.mo_ta || '',
          danh_muc: danhMucIdToIncidentType(report.danh_muc_id),
          vi_do: parseFloat(String(report.vi_do)),
          kinh_do: parseFloat(String(report.kinh_do)),
          dia_chi: report.dia_chi || '',
          uu_tien: uuTienIdToSeverity(report.uu_tien_id),
          la_cong_khai: report.la_cong_khai ?? true,
          media_ids: report.hinh_anhs?.map((m) => m.id) || report.media?.map((m) => m.id) || [],
        });
        if (report.hinh_anhs?.length) {
          const list: IncidentFormMedia[] = report.hinh_anhs.map((img) => {
            const loai = img.loai_file || '';
            const isVideo =
              loai.toLowerCase().includes('video') ||
              /\.(mp4|mov|webm|mkv)(\?|$)/i.test(img.duong_dan_hinh_anh || '');
            return {
              id: img.id,
          ...
          setUploadedMedia(report.media as IncidentFormMedia[]);
        }
      } catch {
        if (!cancelled) {
          setErrorMessage('Không thể tải thông tin phản ánh');
  ...
  const submitDisabled = initialLoad || loading || uploadingMedia || aiAnalyzing;
